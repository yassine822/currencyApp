...
def open_calculator():
    ...
    entry.grid(row=0, column=0, columnspan=4, ipadx=2, ipady=15, padx=20, pady=20)
    ...
    for (text, row, col) in buttons:
        ...
        tk.Button(calc_win, text=text, width=6, height=2, font=("Arial", 12, "bold"),
                  bg="#27ae60" if text == "=" else "#34495e", fg="white",
                  activebackground="#2ecc71", command=action).grid(row=row, column=col, padx=5, pady=5)

    tk.Button(calc_win, text="C", width=27, height=2, font=("Arial", 12, "bold"),
              bg="#e74c3c", fg="white", activebackground="#c0392b", command=clear).grid(row=5, column=0, columnspan=4, padx=5, pady=5)
    
      # -------------------- Notepad --------------------
    def open_notepad():
        note_win = tk.Toplevel(calc_win)
        note_win.title("Notepad")
        note_win.geometry("400x400")
        note_win.config(bg="#2c3e50")
        note_win.resizable(False, False)

        text_area = tk.Text(note_win, font=("Arial", 12), wrap="word",
                            bg="#ecf0f1", fg="#2c3e50")
        ...
        def save_note():
            try:
                ...
                messagebox.showinfo("Saved", "Note saved as note.txt")
        ...
        btn_frame = tk.Frame(note_win, bg="#2c3e50")
        btn_frame.pack(pady=5)

        tk.Button(btn_frame, text="Save", width=10, font=("Arial", 11, "bold"),
                  bg="#27ae60", fg="white", command=save_note).pack(side="left", padx=10)
        tk.Button(btn_frame, text="Clear", width=10, font=("Arial", 11, "bold"),
                  bg="#e74c3c", fg="white", command=clear_note).pack(side="left", padx=10)
    ...
    for (text, row, col) in buttons:
        if text == "=":
            action = calculate
            color = "#27ae60"  # Green for "="
        else:
            action = lambda x=text: press(x)
            color = "#34495e"
        tk.Button(calc_win, text=text, width=8, height=3, font=("Arial", 13, "bold"),
                  bg=color, fg="white", activebackground="#2ecc71",
                  command=action).grid(row=row, column=col, padx=6, pady=6)

    # Clear button spans full row
    tk.Button(calc_win, text="C", width=35, height=3, font=("Arial", 13, "bold"),
              bg="#e74c3c", fg="white", activebackground="#c0392b",
              command=clear).grid(row=5, column=0, columnspan=4, padx=8, pady=12)

    # -------------------- Notepad Icon --------------------
    try:
        note_icon = Image.open("note.png")  # Use a notepad icon image (e.g., note.png)
        note_icon = note_icon.resize((24,24), Image.Resampling.LANCZOS)
        note_photo = ImageTk.PhotoImage(note_icon)
        tk.Button(calc_win, image=note_photo, command=open_notepad,
                  bg="#2c3e50", relief="flat", activebackground="#34495e").place(x=310, y=10)
        calc_win.note_photo = note_photo  # prevent garbage collection
    except:
        # Fallback text button if no icon
        tk.Button(calc_win, text="📝", font=("Arial", 14),
                  command=open_notepad, bg="#2c3e50", fg="white",
                  relief="flat", activebackground="#34495e").place(x=5, y=8)
...
# ---------- Title with Coin ----------
title_frame = tk.Frame(root, bg="#37a36d")
...
# ---------- Calculator Icon ----------
try:
    calc_icon = Image.open("calculator.png")  # <-- place a nice calculator icon in project folder
    calc_icon = calc_icon.resize((28,28), Image.Resampling.LANCZOS)
    calc_photo = ImageTk.PhotoImage(calc_icon)
    calc_btn = tk.Button(title_frame, image=calc_photo, command=open_calculator,
                         bg="#37a36d", bd=0, activebackground="#37a36d")
    calc_btn.pack(side="left", padx=(10,0))
except Exception as e:
    print("Calculator icon missing:", e)
    calc_btn = tk.Button(title_frame, text="🧮", command=open_calculator,
                         bg="#37a36d", fg="white", font=("Arial", 14, "bold"), bd=0,
                         activebackground="#37a36d")
    calc_btn.pack(side="left", padx=(10,0))
...
def on_enter(e): convert_btn['bg'] = '#2ecc71'
def on_leave(e): convert_btn['bg'] = '#27ae60'
...
switch_btn = tk.Button(frame, text="⇄ Switch", command=switch_currencies,
                       ...
                       padx=10, pady=3, relief="raised", bd=2, activebackground="#197132")
